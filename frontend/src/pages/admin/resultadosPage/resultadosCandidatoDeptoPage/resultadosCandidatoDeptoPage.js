...
import './resultadosCandidatoDeptoPage.css';

function ResultadosCandidatoDepartamentoPage() {
  ...
  const handleSeleccion = async (e) => {
    ...
    try {
      const res = await fetch(`http://localhost:4000/resultados/candidato/departamento/${idDepto}`);
      const data = await res.json();
      setResultados(data);
    } catch (err) {
  ...
  return (
    <div className="habilitar-wrapper">
      ...
      <h2 className="subtitulo">Resultados por Candidato - Departamento</h2>
...
export default ResultadosCandidatoDepartamentoPage;
